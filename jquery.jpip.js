...
function jpipViewer(options) {
	...
	this.Load = function() {
		...
		$.get(url, function(data) {
			...
			maxWidth = parseInt(data.width);
			this.maxHeight = parseInt(data.height);
			this.numLevels = parseInt(data.levels);
			this.level = this.numLevels;
			Setup();
		})
		
		.error(function() { 
			document.getElementById("error").innerHTML = "Unable to retrieve image metadata!"});
		*/
	}
	...
	function CreateNavigationWindow() {
		...
		//Add events to the main divs
		//$('#navigationImage').click(scrollNavigation());
		//$('#viewport').dblclick(zoom());
